...
from .utils import read_annotations_bdf
...
class NBack(BaseDataset):
    ...
    def get_raw(self, path, bad_path=None):
        ...
        for f in path:
            try:
                raw = read_raw_bdf(os.path.join(f, 'data.bdf'), preload=True)
                annotation = read_raw_bdf(os.path.join(f, 'evt.bdf'),
                                          preload=True)
                files.append((raw, annotation))
        ...
        for raw, annotation in files:
            tal_data = annotation._read_segment_file(
                np.empty((0, annotation.n_times)), idx, 0, 0,
                int(annotation.n_times), np.ones((len(idx), 1)), None)
        ...
        raw = concatenate_raws([i for i, _ in files])
...
class OneBack(NBack):
    ...
    def _event_tranform(self, events_old, event_id_old):
        ...
        '''
        event_id_new = {'One': 0}
        events_new = pick_events(
            events_old, include=[event_id_old['10'], event_id_old['11']])

        events_new[:, -1][events_new[:, -1] ==
                          event_id_old['10']] = event_id_new['One']
        events_new[:, -1][events_new[:, -1] ==
                          event_id_old['11']] = event_id_new['One']
...
class TwoBack(NBack):
    ...
    def _event_tranform(self, events_old, event_id_old):
        ...
        events_new[:, -1][events_new[:, -1] ==
                          event_id_old['10']] = event_id_new['Two']
        events_new[:, -1][events_new[:, -1] ==
                          event_id_old['11']] = event_id_new['Two']
